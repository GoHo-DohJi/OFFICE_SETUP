...
function Write-Centered {
    ...
    if ($DownloadPrompt) {
        Write-Host ""
        Write-Host (Center "[D] DOWNLOAD IN BROWSER   [ENTER] SELECT IMAGE FILE") -ForegroundColor "DarkYellow"
        Write-Host ""

        $StatusY = [Console]::CursorTop
        [Console]::CursorVisible = $false
        ...
        while ($true) {
            switch (($Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")).VirtualKeyCode) {
                68 {  # D
                    ...
                    # $msg = Center "✅ DOWNLOAD STARTED IN DEFAULT BROWSER"
                    # Write-Host $msg.PadRight($Host.UI.RawUI.WindowSize.Width) -ForegroundColor "Green"
                }
                13 {  # Enter
                    [Console]::CursorVisible = $true
                    return
